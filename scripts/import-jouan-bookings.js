// استيراد حجوزات جوان ان من بيانات نزيل
...
const RAW_FILE = path.join(__dirname, 'jouan-bookings-raw.tsv');
const BUILDING = 'جوان ان';
const PROGRESS = path.join(__dirname, 'jouan-bookings-progress.json');
...
function parseLine(l) {
  ...
  return {
    ...
    total:      parseFloat(ap[5]) || 0,
    paid:       parseFloat(ap[7]) || 0,
  };
}
...
async function run() {
  ...
  if (fs.existsSync(PROGRESS)) {
    try {
      ...
      console.log(`📋 استكمال من الحجز رقم ${startIdx} (تم: ${added} إضافة، ${skipped} تخطي)\n`);
    } catch (_) {}
  }
  ...
  for (let i = startIdx; i < bookings.length; i += BATCH) {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      if (res.status === 'fulfilled') {
        ...
        errors++;
        console.error(`  ⚠️ ${chunk[j].bookingNum} (${chunk[j].guestName}) — ${res.reason?.message}`);
      }
    }

    // طباعة تقدم كل 50 حجز
    if ((i + BATCH) % 50 < BATCH) {
